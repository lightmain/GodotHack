...
- `Save and Exit`：关闭暂停层并向原输入请求发送 ASCII `S`，只对紧随其后的
  精确 `Really save?` 请求自动回答 `y`，并自动确认随后唯一一次阻塞消息窗口，
  继续复用 NetHack 的保存、退出和 IDBFS flush 流程。玩家直接按 `S` 时仍显示
  原生确认和 `--More--`。
...
- Save and Exit 向原输入发送 `S`，一次性自动回答对应保存确认和随后阻塞消息，
  并复用原生保存和退出流程。
...
- Save and Exit 除自动回答对应确认和随后阻塞消息外，与键盘 `S` 产生相同的
  保存、flush 和返回 Home 流程；自动化不得影响其他 `yn` 或 display 请求。
- 恢复旧存档的初始快照不会覆盖个人默认值。
- 图形修改和原生命令修改双向同步。
